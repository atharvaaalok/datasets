...
def su2_simulation_func(
    ...
    template_cfg_file_path = Path(__file__).resolve().parent / f'su2_{type_of_simulation}_template.cfg'
    ...
    with open(os.devnull, 'w') as devnull:
        subprocess.run(['SU2_CFD', cfg_file_path.name], check = True,
                       cwd = cfg_file_path.parent, stdout = devnull, stderr = subprocess.STDOUT)
